...
3. **Test coverage is thin outside known-fragile areas.** 172 JVM tests cover workout logger, cycle progress, history filtering, program-editor helpers, progression hints/projection, cycle-phase classification, PR detection (`PrService.detectNewPRs`), and cycle comparison (`AnalyticsRepository.compareCycles`); dashboard widgets, remaining analytics flows (volume/trend/PR-progression), import/export, and cloud backup are still validated only by running the app.
